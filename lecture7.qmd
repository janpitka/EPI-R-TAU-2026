---
title: "Epidemiological Data Analysis using R - Population-based cancer survival"
author: 
  - Karri Seppä \newline
...
1.  Relative survival estimators \medskip
2.  Age-standardised relative survival \medskip
3.  Period approach \medskip
4.  Regression modelling of relative survival.

-   Packages \texttt{popEpi} and \texttt{rstpm2}. \medskip
...
-   Total mortality among patients $\lambda(t)$ is the sum of population mortality $\lambda_\text{P}(t)$ and excess mortality $\lambda_\text{E}(t)$ due to cancer: $$\lambda(t)=\lambda_\text{P}(t) + \lambda_\text{E}(t)$$

-   Relative survival $S_E(t)$ is the survival function related to the excess mortality: $$S_\text{E}(t)=\exp\left[ - \int_0^t \lambda_\text{E}(u)\ du \right] $$

    -   Often interpreted as the survival **probability** in a hypothetical situation where the cancer would be the only possible cause of death with all other causes being eliminated.
...
-   Ederer II (Ederer and Heise 1958) $$S_\text{E,EII}(t_k)=\exp\left(-\sum_{j=1}^k\Delta_j\frac{d_j-d_j^*}{y_j} \right)$$
...
-   Pohar Perme estimator (Pohar Perme et al. 2012) $$S_\text{E,PP}(t_k)\exp\left(-\sum_{j=1}^k\Delta_j\frac{d_j^w-d_j^{*w}}{y_j^w} \right)$$
    -   $d_j^w$, $d_j^{*w}$ and $y_j^w$ are based on individual-level observations of $d_j$, $d_j^*$ and $y_j$ weighted by the patient-specific cumulative expected survival probability (see Seppä et al. 2016).
...
-   Both the Pohar Perme and the age-standardised Ederer II estimator are valid for 5-year net survival of cancer patients.
...
# Age standardisation in sparse data

-   Use pre-weighting of individual observations instead of weighted average of age-specific estimates

-   Allows estimation up to 5 years, even though the 5-year estimate would not be available for all age groups

-   See: Rutherford et al. 2020: Estimation of age-standardized net survival, even when age-specific data are sparse.

```{r, comment=NA,echo=FALSE,results='hide',warning=FALSE,message=FALSE}
...
Lex <- Lexis(entry = list(FUT = 0, AGE = dg_age, CAL = dg_date), 
           exit = list(FUT = time), 
           data = orca,
           exit.status = factor(event, levels = 0:2, 
...
weights <- w[,c(1,2)]
weights$N <- table(Lex$agegr)
weights$w_pat <- weights$N/sum(weights$N)
weights$w_ICSS1 <- weights$ICSS1/100000
weights$individual_weights <- weights$w_ICSS1/weights$w_pat
```
...
\footnotesize
\vspace{-0.1cm}
```{r, comment=NA,echo=T,warning=FALSE,message=FALSE}
orca <- read.table(file="oralca2.txt")
library(Epi)
library(popEpi)
set.seed(23032020)
...
```{r, comment=NA,echo=T,message=F,fig.height=5.0, dev.args=list(pointsize=23)}
par(mar=c(4,4,0.5,1))
...
```{r, comment=NA,echo=T,message=F,fig.height=5.0, dev.args=list(pointsize=23)}
par(mar=c(4,4,0.5,1))
plot(surv.PP, lwd=2, conf.int=F,ylim=c(0,1)) 
lines(surv.PP.ICSS, lwd=2, lty=2, conf.int=F)
legend("bottomleft",c("Non-standardised Pohar Perme",
                    "Age-standardised Pohar Perme (ICSS weights)"),
       lty=c(1,2), lwd=2,col=c("black","black"))
```
...
-   Use period approach (Brenner et al. 2004) to produce maximally updated estimates of patient survival
...
#Age-standardised Pohar Perme using ICSS weights and period approach
surv.PP.ICSS.period <- survtab(FUT ~ 1+adjust(agegr), data = Lex, 
              surv.type = "surv.rel",relsurv.method = "pp",
              pophaz = pm, weights = w$ICSS1,
...
```{r, comment=NA,echo=T,message=F,fig.height=5.0, dev.args=list(pointsize=20)}
#Age-standardised Ederer II estimator
par(mar=c(4,4,0.5,1))
plot(surv.PP.ICSS, lwd=2, conf.int=F,ylim=c(0,1)) 
lines(surv.PP.ICSS.period, lwd=2,col="red", conf.int=F)
...
1.  Logarithm of excess mortality $$\log[\lambda_\text{E}(t)]=\alpha+\beta X$$
...
fit3 <- stpm2(Surv(time,event%in%c(1,2))~1,
              data=orca,df=3,bhazard=orca$haz)
```
