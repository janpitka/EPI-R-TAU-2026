...
The diet data frame has 337 rows and the following 15 columns:
...
(a) Load the diet to R data frame, see what's in there: 
...
(b) The outcome event of interest is the first occurrence of coronary heart disease, this variable being named chd and coded 1 and 0. The person-time variable is y from the individual date of entry until the date of exit from the follow-up.
...
D <- sum( diet$chd ) ; #sum of the CHD events
Y <- sum( diet$y ) ;  # sum of the person time at risk of CHD
...
(e) Find out how the rates vary between the two levels of energy.grp. Tabulate cases, person-years, and rates by energy.grp but save the table into an object with nice annotations:
...
paste("rate ratio =",rate[1]/rate[2])
```
...
meh2 <- update(meh, . ~ . + eg2:htgrp )  ## (eg2 + htgrp + eg2:htgrp) = eg2*htgrp 
...
diet$hei.quad <- diet$hei.lin^2 #use centered and scaled version of height instead of original
meh4 <- update(meh3b, . ~ . + hei.quad)
round(ci.lin(meh4, Exp=T)[ , -(3:4)], 4)
anova(meh3b, meh4, test="LRT")
```
