...
*   split times into agebands and expanding the data frame using functions Lexis(), splitLexis() and timeBand() in the Epi package,
*   model-based adjustment for current age and evaluation of modification in rate ratio estimation,
*   graphical display of age-specific rates using plot() function, and estimation results from models using plotEst() in Epi,
*   estimate age-standardised incidence rate using rate() function in popEpi() package.
...
(d) Rename also the ageband-specific event indicator `dietA$lex.Xst` to be `dieatA$d_ik` and the ageband-specific
person-time slot `dietA$lex.dur` to be `dietA$y_ik`, analogous to the notation used in the lecture notes:
...
(e) A check of the overall no. of cases and person-years:
...
Instead of the concise representation eg2*ageband covering both the main effects and the interaction we could write more explicitly eg2 + ageband + eg2:ageband in the model formula. View the results:
...
dietA$agebyrs[dietA$agebnum==1] <- 40  #mid-point of the first ageband (wider)
table(dietA$ageband, dietA$agebyrs)
```

(c) However, very often it is more advisable to use centered values for a quantitative variable. For example, instead of age values 1, 2, and 3, we could recode these into -1.5; 0; and 1. These values define a linear contrast between the ageband levels:

```{r p4_32, eval=TRUE,echo=TRUE,warnings=FALSE}
dietA$agebcontr <- dietA$agebnum - 2
dietA$agebcontr[dietA$agebnum==1] <- -1.5  #the first ageband is wider
...
table(dietA$ageband, dietA$ageby55)
```

Actually, the values -1.5; 0; 1 represent a centered and scaled version of the age values 40, 55, and 65; they are centered at 55 years and scaled by 10 years. 
...
(e) We shall now fit a model with continuous age using the centered and scaled version of it, i.e. the contrast coding -1.5; 0; 1:
...
plot(as.numeric(tab1$year),tab1$rate*100000, type="l",ylim=c(0, 35),
     xlab="Year", ylab="Incidence per 100 000 person years")
