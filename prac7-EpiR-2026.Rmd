...
Lex_aggre <- lexpand(sire, birth = bi_date,
...
m1 <- relpois_ag(formula = from0to1 ~ period+fot*agegr, 
                 data = Lex_aggre, d.exp = d.exp, offset = log(pyrs))
ci.exp(m1)
```
...
EXTRA: The same model can be fitted using ``glm`` function. The link function of relative survival Poisson regression can be specified as follows:

```{r p9_13, echo=TRUE, message=F}
Lex_aggre$FUT <- cut( Lex_aggre$fot, c(0,3/12,1:5), right=F)
d_star <- Lex_aggre$d.exp
...
assign(".d_star", Lex_aggre$d.exp, env = .GlobalEnv)
...
m2 <- glm(formula = from0to1 ~ period+FUT*agegr, 
          data = Lex_aggre, family=rspois, offset = log(pyrs))
ci.exp(m2)
```


# Optional: pre-weighting approach for age standardisation of relative survival

In sparse data (in this example: a small subset of patients diagnosed in 2003-2012), pre-weighting approach allows estimation of longer term net survival. The weights are defined as follows:  

```{r opt1, echo=TRUE, message=F}
#The weights for individual observations
sire2003 <- sire[period=="[2003,2013)" & dg_date<ex_date][seq(1,2000,20),]
weights <- table(sire2003$agegr[sire2003$period=="[2003,2013)"])
weights <- data.frame(weights,w)
colnames(weights) <- c("agegr","N","ICSS1")
...
Merge patient data with individual weights and split follow-up time into monthly intervals:

```{r opt2, echo=TRUE, message=F}
sire2003 <- merge(sire2003,weights[,c("agegr","individual_weights")],by="agegr")

BL <- list(fot=seq(0, 25, by = 1/12))
Lex_aggre_ind_w <- lexpand(sire2003, birth = bi_date,
               entry = dg_date, exit = ex_date,
               status = status%in%1:2,
               breaks = BL, pophaz = pm,pp=T,
               aggre = list(fot,agegr,individual_weights)
)
```

Estimate age-standardised net survival using a) traditional weighting (i.e. weighted average of age-specific estimates) and b) pre-weighting approach

```{r opt3, echo=TRUE, message=F}
#Traditional age-standardisation (stratification)
st.pp.as <- survtab_ag(fot ~ adjust(agegr), data = Lex_aggre_ind_w,     
         weights = weights$w_ICSS1,
         surv.type = "surv.rel", relsurv.method = "pp",
         d.pp = "from0to1.pp", d.pp.2 = "from0to1.pp.2", 
         pyrs.pp = "ptime.pp", d.exp.pp = "d.exp.pp")

#Pre-weighting of observations
Lex_aggre_ind_w[,from0to1.pp.prew:=from0to1.pp*individual_weights]
Lex_aggre_ind_w[,from0to1.pp.prew.2:=from0to1.pp.2*(individual_weights^2)]
Lex_aggre_ind_w[,ptime.pp.prew:=ptime.pp*individual_weights]
Lex_aggre_ind_w[,d.exp.pp.prew:=d.exp.pp*individual_weights]
st.pp.prew <- survtab_ag(fot ~ 1, data = Lex_aggre_ind_w, 
         surv.type = "surv.rel", relsurv.method = "pp",
         d.pp = "from0to1.pp.prew", d.pp.2 = "from0to1.pp.prew.2", 
         pyrs.pp = "ptime.pp.prew", d.exp.pp = "d.exp.pp.prew")

plot(st.pp.prew)
lines(st.pp.as,col="red")
```

Note: The pre-weighting approach allows estimation of net survival for a longer period of follow-up than the traditional age standardisation.
