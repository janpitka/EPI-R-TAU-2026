...
Empirical **incidence rates** by exposure group: $$ I_1 = {D_1}/{Y_1}, \qquad
    I_0 = {D_0}/{Y_0}. $$ These provide estimates for the true {\bf hazards} (or hazard rates) $\lambda_1$ and $\lambda_0$ assumed constant within exposure categories.
...
-   **hazard ratio** $$ \rho = \frac{\lambda_1}{\lambda_0} = 
    \frac{hazard  among  exposed } {hazard  among  unexposed}. $$
...
-   one categorical (polytomous) factor $Z$.
    -   *Random part*: No. of cases in exposure group $j$ $(j=0,1)$ and covariate level $k$ $(k = 1, \dots, K)$ is $D_{jk} \sim Poisson( \lambda_{jk} Y_{jk} )$
...
From the estimated coefficients we can calculate \newline **fitted** linear predictors $\widehat \eta_{jk}$, hazards $\widehat \lambda_{jk}$ and numbers $\widehat \mu_{jk}$: $$ \widehat \eta_{jk}  =  \widehat\alpha + \widehat\beta x + \widehat\gamma_k $$ $$ \widehat \lambda_{jk}  =  \mbox{exp}(\widehat \eta_{jk}), \quad \widehat\mu_{jk} = \widehat\lambda_{jk} Y_{jk} $$
...
**Deviance residual** for cell $jk$ (`resid(m2)` in R): $$ d_{jk}  =  \mbox{sign}(Y_{jk} - \widehat\mu_{jk}) 
                    \times 
                   \sqrt{ 2 \left\lbrace
...
-   Both Dev and $X^2$ are approximately $\chi^2$-distributed with *degrees of freedom* (df) being equal to the \newline *residual* df = no. of obs -- no. of coeff's
...
-   Misspecified systematic part? - Wrong functional form for effects? \medskip - Missing important risk factors? \medskip - Failure to take into account *effect modification* or "interaction".
