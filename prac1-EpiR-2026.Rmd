...
5.  Check whether there are any objects in the memory from the upper risgt hand corner - Environment
...
3.  Run the commands written to the script file as follows: click the Run command in the scritping window
...
Do tabulations using the *stat.table()* function in the *Epi* package. First install package *Epi* from R-studio *Tools -- install packages* and run following scrpit
...
6.  Assign the value of the chi-square test function in the previous item to an object with name *res*, say, and view its structure usint *str()* function.
...
Create and print by *stat.table()* a 2-way frequency table with *age.gr* as the row variable and variable *LN_YESNO* (presence vs. absence of $\geq 1$ lymph nodes) as the column variable. Present the row percentages, too.
...
If you managed to do the previous tasks within the time allocated, get familial with data.table functions
